...
class Cardinal(object):
    ...
    def __get__(self, obj, objtype):
        logger.debug(
            "Cardinal.get: self={!r}, obj={!r}, objtype={}".format(self, obj, objtype)
        )
        return self.data.get(obj, self.default)

    def __set__(self, obj, value):
        logger.debug(
            "Cardinal.set: self={!r}, obj={!r}, value={!r}".format(self, obj, value)
        )
...
class Movie(object):
    ...
    def __init__(self, title, budget, gross):
        logger.debug(
            "Movie.init: title={title!r}, budget={budget!r}, gross={gross!r}".format(
                **locals()
            )
        )
