...
def set_cookie(obj):
    ...
    response.set_cookie(COOKIE_NAME, obj, secret=SECRET, httponly=True, path="/")


def make_html_page(content):
    return (
        "<html><head><title>SmugMug OAuth Demo</title></head><body>"
        + content
        + "</body></html>"
    )
...
def make_api_request(session, input_path):
    ...
    params["_pretty"] = ""
    return session.get(
        API_ORIGIN + parsed.path, params=params, headers={"Accept": "application/json"}
    ).text
