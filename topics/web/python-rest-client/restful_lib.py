...
class Connection:
    ...
    def request(self, resource, method = "get", args = None, body = None, filename=None, headers={}):
        ...
        BOUNDARY = '00hoYUXOnLD5RQ8SKGYVgLLt64jejnMwtO7q8XE1'
        CRLF = '\r\n'
        ...
        if filename and body:
            ...
            encode_string.write('--' + BOUNDARY + CRLF)
            encode_string.write('Content-Disposition: form-data; name="file"; filename="%s"' % filename)
            encode_string.write(CRLF)
            encode_string.write('Content-Type: %s' % content_type + CRLF)
            encode_string.write(CRLF)
        ...
        return {'headers':resp, 'body':content.decode('UTF-8')}
