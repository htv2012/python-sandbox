...
class Sudoku:
    ...
    def next_unsolved_coordinate(self):
        return next((c for c in all_coordinates if self.grid[c] ==  EMPTY), None)

    def find_candidates(self, coordinate):
        candidates = set('123456789') - set(self.grid[c] for c in neighbors[coordinate])
        return candidates
...
""")
print(sudoku)
solved = sudoku.solve_by_brute_force()
print(sudoku)
